...
class MyAdjacencyMatrix extends AdjacencyMatrix {
  ...
  private addNewBatch (i: number): void {
    for (let j = 1; j < this.vertexNum; j++) {
      ...
      const visitedCost = this.visited[j]?.cost || Infinity;
      if (currCost < visitedCost) {
  ...
  private getNearestVertex(): number {
    ...
    for (let j = 1; j < this.vertexNum; j++) {
      ...
      if (!vertex || vertex.done) continue;
